...
for i, ticker in enumerate(eq_sm_df["ticker"].tolist()):
    yahoo_obj = YahooStockParser(
        ticker, datetime.date(2020, 7, 1), datetime.date(2020, 7, 19)
    )
...
profiles_df.to_sql(
    "eq_company_profile", engine, index=False, dtype=dtype_dict, if_exists="append"
)
