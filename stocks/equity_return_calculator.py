...
class EquityReturnCalculator:
    ...
    def get_eq_prices(self) -> pd.DataFrame:
        ...
        logging.info(
            f"Retreived {len(df)} records for {self.ticker} from {self.start} to {self.end}"
        )
        return df
    ...
    def generate_equity_returns(self, eq_px_df: pd.DataFrame) -> pd.DataFrame:
        ...
        eq_px_df.loc[1:, "previous_price"] = eq_px_df.loc[
            0 : eq_px_df.index[-2], "price"
        ].tolist()
        eq_px_df.loc[1:, "previous_date"] = eq_px_df.loc[
            0 : eq_px_df.index[-2], "adate"
        ].tolist()
        ...
        logging.info(
            f"Finished calculating daily returns for {self.ticker} from {self.start} to {self.end}"
        )
        return eq_px_df.loc[
            1:,
            [
    ...
    def save_to_db(self, eq_ret_df: pd.DataFrame):
        ...
        logging.info(
            f"Finished inserting {len(eq_ret_df)} daily returns into {self.eq_ret_table}"
        )
