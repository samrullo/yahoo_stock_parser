...
_logger = setup_custom_logger(
    __file__, "nikkei225_scraping.log", log_level=logging.INFO
)
...
for row in nk225_table:
    ...
    if record:
        ...
        if "" in aset:
            aset.remove("")
        _logger.info(
            "After converting to set and removing empty strings:{}".format(aset)
        )
        if len(aset) == 1:
            current_industry = aset.pop()
            _logger.info(
                "The record contains industry_name:{}".format(current_industry)
            )
        elif len(aset) == 3:
            record_list = list(aset)
            record_list.sort()
            _logger.info(
                "This was regular record:{}".format(record_list + [current_industry])
            )
