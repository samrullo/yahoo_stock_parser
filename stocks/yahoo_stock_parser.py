...
def construct_url(
    ticker: str, start_date: datetime.date, end_date: datetime.date, page_num: int
) -> str:
    """
...
class YahooStockParser:
    def __init__(self, ticker: str, start: datetime.date, end: datetime.date):
        ...
        self.stock_px_table_class_name = (
            "StocksEtfReitPriceHistory__historyTable__13C_ HistoryTable__1aNP"
        )
        self.profile_url = (
            f"https://stocks.finance.yahoo.co.jp/stocks/profile/?code={self.ticker}"
        )
        self.profile_fields = {
            "特色": "features",
            "連結事業": "consolidated_business",
            "本社所在地": "headquarters",
            "最寄り駅": "closest_station",
            "電話番号": "phone",
            "業種分類": "industry",
            "英文社名": "company_name_en",
            "代表者名": "ceo",
            "設立年月日": "founded_date",
            "市場名": "listed_market",
            "上場年月日": "ipo_date",
            ...
            "従業員数（単独）": "number_of_employees_single",
            "従業員数（連結）": "number_of_employees_consolidated",
            "平均年齢": "average_age",
            "平均年収": "average_annual_salary",
        }
    ...
    def get_date_from_japanese_date(self, jp_date: str) -> datetime.date:
        ...
        return datetime.date(
            self.get_number_part_of_jp_date_token(jp_date, "年"),
            self.get_number_part_of_jp_date_token(jp_date, "月"),
            self.get_number_part_of_jp_date_token(jp_date, "日"),
        )

    def convert_comma_seperated_number_string_to_integer(
        self, stock_px_str: str
    ) -> int:
        """
    ...
    def convert_comma_seperated_number_string_to_float(
        self, stock_px_str: str
    ) -> float:
        """
    ...
    def extract_data_from_stock_px_row(
        self, stock_px_row: bs4.element.Tag
    ) -> Tuple[datetime.date, float, float, float, float, int, float]:
        """
        ...
        if len(stock_columns) == 6:
            px_open = self.convert_comma_seperated_number_string_to_float(
                stock_columns[0].get_text()
            )
                ...
                for i, stock_row in enumerate(stock_rows[1:]):
                    try:
                        (
                            ...
                            volume,
                            px_close_after_adj,
                        ) = self.extract_data_from_stock_px_row(stock_row)
                        ...
                        logging.info(
                            f"Something went wrong processing {i+1}th row of {self.ticker}\n{e}"
                        )
        return stock_df
            ...
            if len(ths) > 1:
                if "従業員数" in ths[0].get_text():
                    ...
                    if number_of_employees_single == "-":
                        ...
                        profile["number_of_employees_single"] = (
                            self.convert_comma_seperated_number_string_to_integer(
                                number_of_employees_single
                            )
                        )
                        profile["number_of_employees_consolidated"] = profile[
                            "number_of_employees_single"
                        ]
                    else:
                        profile["number_of_employees_single"] = (
                        ...
                        profile["number_of_employees_consolidated"] = (
                            self.convert_comma_seperated_number_string_to_integer(
                                number_of_employees_consolidated
                            )
                        )
                ...
                if "平均" in ths[0].get_text():
                    ...
                    if "‐" not in average_age and "-" not in average_age:
                        profile["average_age"] = (
                            self.convert_comma_seperated_number_string_to_float(
                                average_age
                            )
                        )
                    else:
                    ...
                    if (
                        "‐" not in average_annual_salary
                        and "-" not in average_annual_salary
                    ):
                        profile["average_annual_salary"] = (
                            self.convert_comma_seperated_number_string_to_integer(
                ...
                if "設立年月日" in ths[0].get_text():
                    ...
                    if "日" not in founded_date_str:
                        founded_date_str += "1日"
                    profile["founded_date"] = self.get_date_from_japanese_date(
                        founded_date_str
                    )
                    ...
                    if "日" not in ipo_date_str:
                        ipo_date_str += "1日"
                    logging.debug(
                        f"trying to convert ipo date {ipo_date_str} to datetime.date"
                    )
                    ...
                    profile["number_of_shares_per_transaction"] = int(
                        tds[0].get_text().strip().replace("株", "")
                    )
                    logging.debug(
                        f"number of shares per transaction : {profile['number_of_shares_per_transaction']}"
                    )
                else:
                    profile[self.profile_fields[ths[0].get_text().strip()]] = (
