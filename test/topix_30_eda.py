...
pvt_df = pd.pivot_table(eq_ret_df, index=['ticker', 'desc_en', "component_weight"], values=['daily_ret'], aggfunc=[np.mean, np.std])
logging.info(f"pivoted df {len(pvt_df)}")

eq_df = pd.DataFrame(index=pvt_df.index.get_level_values('ticker').tolist(), data={'desc': pvt_df.index.get_level_values('desc_en').tolist(), 'daily_ret_mean': pvt_df['mean']['daily_ret'].tolist(), 'daily_ret_std': pvt_df['std']['daily_ret'].tolist(), 'weight': pvt_df.index.get_level_values('component_weight').tolist()})
...
g = sns.relplot(x="daily_ret_std_bps", y="daily_ret_mean_bps", data=eq_df, kind="scatter", size="weight")
...
for i, row in eq_df.iterrows():
    ...
    ax.text(x_coord + 0.05, y_coord, point_label, horizontalalignment='left',fontsize=9)
plt.show()
